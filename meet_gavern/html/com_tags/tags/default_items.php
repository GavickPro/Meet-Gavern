	...
	<?php foreach ($this->items as $i => $item) : ?>
		...
		<?php if ((!empty($item->access)) && in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
 			<li class="cat-list-row<?php echo $i % 2; ?>" >
				<h3>
					<a class="tag" data-toggle="popover" data-tag-content="
					...
					<?php endif; ?>" data-trigger="hover" data-img-url="<?php if ($this->params->get('all_tags_show_tag_image') && !empty($item->images)) : ?><?php $images  = json_decode($item->images); ?><?php if (!empty($images->image_intro)): ?><?php echo JURI::root().$images->image_intro; ?><?php endif; ?><?php endif; ?>" data-original-title="<?php echo $this->escape($item->title); ?>" href="<?php echo JRoute::_(TagsHelperRoute::getTagRoute($item->id . ':' . $item->alias)); ?>">
